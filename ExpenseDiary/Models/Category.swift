...
class Category: Object, Identifiable {
    ...
    static func seed() {
        let categories = [
            Category(value: ["type" : RecordType.expense.rawValue, "name" : "合計", "icon" : Icon.find("all")!, "order" : 0, "total" : 1]),
           ...
           Category(value: ["type" : RecordType.income.rawValue, "name" : "賞与", "icon" : Icon.find("nasu")!, "order" : 2])
        ]
